   ...
   //
	...
	case WM_SEC_UPDATE:
		...
		if (gui_Vars.prevSecond_L == 9)
		{
			sprintf(timeString, ":%d", sTime.Seconds / 10);
			TEXT_SetText(handles.hSecondsString_H, timeString);
			if (gui_Vars.prevSecond_H == 5) //ïðîøëà ìèíóòà
			{
				sprintf(timeString, "%02d:%02d", sTime.Hours, sTime.Minutes);
				TEXT_SetText(handles.hHourMinString, timeString);      //îáíîâëåíèå ñòðîêè ñ ÷àñàìè è ìèíóòàìè
...
void TFT_MainMenu_ShowLineTime()
{
	...
	//for (i = 0; i < 4; i++)
	//{
	//	TFT_ReadBackground(TEXT_TIME_LINE_X - 8 + 80 * i, 209, 18, backgroundBuffer);      //80-øèðèíà êíîïîê, 2-îòñòóï, ÷òîáû ïîïàñòü òî÷íî íà ôîí
	//	switch (line[i].Status) {
	//	case LINE_STATUS_OFF:
	//		TFT_ShowString(TEXT_TIME_LINE_X - 5 + 80 * i, 209, "ÂÛÊË ", 18, 0x8410);     //öâåò 0x8410 BGR
	//		break;
	//	case LINE_STATUS_RUN:
	...
	//		break;
	//	case LINE_STATUS_STOP:
	//		TFT_ShowString(TEXT_TIME_LINE_X - 5 + 80 * i, 209, "ÑÒÎÏ ", 18, 0x1F);     //0xF800
	//		break;
	//	}
	//}
...
void TFT_MainMenu_ShowLineTime_ex(uint8_t i)
{
	...
	//switch (line[i].Status) {
	//case LINE_STATUS_OFF:
	//	TFT_ShowString(TEXT_TIME_LINE_X - 5 + 80 * i, 209, "ÂÛÊË ", 18, 0x8410);
	//	break;
	//case LINE_STATUS_RUN:
	//	sprintf(timeString, "%02d:%02d", line[i].Hours, line[i].Minutes);
	//	strcat(timeString, " ");
	...
	//	TFT_ShowString(TEXT_TIME_LINE_X - 5 + 80 * i, 209, "ÑÒÎÏ ", 18, 0x1F);
	//	break;
	//}
